...
if($optn==1)
{
    ...
    $mbillno=$_POST['mbillno'];
    $mbilldt=$_POST['mbilldt'];
    $mrem=$_POST['mrem'];
        ...
        if(!empty($mdocno))
        {
            try
            {
                $mysql_query="insert into bills(
                    ...
                    gln,sdes,sttype,billno,billdt,
                    rem,vchno,amount1,dis,misc,
                ...
                $arr=array(
                    ...
                    $mgln,$msdes,$msttype,$mbillno,$mbilldt,
                    $mrem,$mvchno,$mamount1,$mdis,$mmisc,
                ...
                if($mysql_query_run)
                {
                    ...
                    foreach($array as $prop=>$val)
                    {
                        ...
                        (tc,ser,docno,docdt,vchno,pcode,pname,qty,rate,less,gamount,gper1,gper2,gper3,gstax1,gstax2,gstax3,amount,sdes,sttype,glc,
                        gln,billno,billdt,uname,gstdes,gsttype,unit,pglc,pgln)
                        values(
                            ?,?,?,?,?,
                            ?,?,?,?,?,
                            ?,?,?,?,?,
                        ...
                        $arr=array($mtc,$mser,$mdocno,$mdocdt,$mvchno,$mpcode,$mpname,$mqty,$mrate,$mless,$mgamount,$mgper1,$mgper2,$mgper3,
                        $mgstax1,$mgstax2,$mgstax3,$amount,$msdes,$msttype,$mglc,
                        $mgln,$mbillno,$mbilldt,$mcuid,$mgstdes,$mgsttype,$munit,$mpglc,$mpgln);
...
if($optn==2)
{
    ...
    try
    {
        $mysql_query="update bills set docdt=?,glc=?,gln=?,sdes=?,sttype=?,billno=?,billdt=?,rem=?,
        amount1=?,dis=?,misc=?,sgst=?,cgst=?,igst=?,tcsamt=?,billamt=?,bqty=?,taxable=?,pglc=?,pgln=? where tc=? and ser=? and docno=?";
            $arr=array($mdocdt,$mglc,$mgln,$msdes,$msttype,$mbillno,$mbilldt,$mrem,
        ...
        foreach($array as $prop=>$val)
        {
            ...
            foreach($array as $prop=>$val)
            {
                ...
                $arr=array($mtc,$mser,$mdocno,$mdocdt,$mvchno,$mpcode,$mpname,$mqty,$mrate,$mless,$mgamount,$mgper1,$mgper2,$mgper3,
                $mgstax1,$mgstax2,$mgstax3,$amount,$msdes,$msttype,$mglc,$mgln,$mbillno,
                $mbilldt,$mcuid,$mgstdes,$mgsttype,$munit,$mpglc,$mpgln);
                $run=$db->RunQryparam($qry,$arr);
            }
...
if($optn==3)
{
    ...
    $qry="select * from bills where tc='P11' and 1=1 and rtrim(billno)+rtrim(gln) like '%$mdes%'  and glc like '$fglc%' $xdes order by convert(docno,int),docdt";
    // echo $qry;
    if($result=$db->getResult($qry))
    {
        ...
        while($mysql_row=mysqli_fetch_array($result))
        {
            ...
            <tr class='hightlight'>
                ...
                $qry="select * from sale where vchno='$vchno' and tc='P11'";
...
if($optn==5)
{
    ...
    ?>
    <link rel='text/stylesheet' href='purc.css'/>
    <style>
        .itemlist
        {
            display:none;
        ...
        foreach($array as $prop=>$val)
        {
            ...
            echo "
                <tr>
                    ...
                    <td class='toright'>$rate</td>
                    <td class='toright'>$gamount</td>
                    <td class='toright'>$less</td>
                    <td class='toright'>$taxable</td>

                    <td style='width:10px;padding-left:5px;padding-right: 5px;'>";
                        echo "<div class='fa fa-edit mstedit' onclick='fillrecdet(this.id);' id='edit$trackno'
                        data-toggle='tooltip' title='Edit' data-original-title='Edit' data-placement='top'></div>";
                    echo "</td>
                    <td style='width:10px;padding-left:5px;padding-right: 5px;'>";
...
if($optn==6)
{
    ...
    $tamount=0.00;$tdis=0.00;$ttaxable=0.00;$tsgst=0.00;$tcgst=0.00;$tigst=0.00;$ttcs=0.00;$tbillamt=0.00;$tmisc=0.00;
    $qry="select * from sale where tc='P11' and 1=1 and rtrim(billno)+rtrim(gln) like '%$mdes%' and 
